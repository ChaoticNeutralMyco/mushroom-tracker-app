...
export function normalizeStageName(stage = "") {
  ...
  const match = ENVIRONMENT_TARGET_STAGES.find((item) => item.toLowerCase() === clean.toLowerCase());
  return match || clean;
}
...
function pickStageFallback(stage = "") {
  ...
  if (normalized === "Fruiting" || normalized === "Harvesting" || normalized === "Harvested") {
    return "fruiting";
  }
  if (normalized === "Inoculated" || normalized === "Colonizing" || normalized === "Colonized") {
    return "colonization";
  }
  return "general";
}

export function getStageEnvironmentTarget({ prefs = {}, stage = "", cultivationProfile = null } = {}) {
  const normalizedStage = normalizeStageName(stage);
  const allTargets = normalizeEnvironmentTargets(prefs?.environmentTargets || {});
  const globalTarget = allTargets[normalizedStage] || allTargets.General || normalizeEnvironmentTarget({});
  const profile = cultivationProfile && typeof cultivationProfile === "object" ? cultivationProfile : {};
    ...
    tempMaxF:
      fallbackKind === "fruiting"
        ...
        : fallbackKind === "colonization"
          ...
          : "",
    humidityMin: fallbackKind === "fruiting" ? profile.fruitingHumidityMin : "",
    humidityMax: fallbackKind === "fruiting" ? profile.fruitingHumidityMax : "",
    notes:
      fallbackKind === "fruiting"
        ? profile.bulkNotes || profile.contaminationNotes || ""
        : fallbackKind === "colonization"
          ? profile.grainNotes || profile.agarNotes || profile.lcNotes || profile.cleanWorkNotes || ""
          : "",
  });

  const merged = normalizeEnvironmentTarget({
    ...globalTarget,
    ...(hasValue(profileTarget.tempMinF) ? { tempMinF: profileTarget.tempMinF } : {}),
    ...(hasValue(profileTarget.tempMaxF) ? { tempMaxF: profileTarget.tempMaxF } : {}),
    ...(hasValue(profileTarget.humidityMin) ? { humidityMin: profileTarget.humidityMin } : {}),
    ...(hasValue(profileTarget.humidityMax) ? { humidityMax: profileTarget.humidityMax } : {}),
...
export function compareToRange(value, min, max) {
  ...
  if (lo !== null && n < lo) {
    return { status: "low", label: `Low by ${(lo - n).toFixed(1).replace(/\.0$/, "")}`, delta: lo - n };
  }
  if (hi !== null && n > hi) {
    return { status: "high", label: `High by ${(n - hi).toFixed(1).replace(/\.0$/, "")}`, delta: n - hi };
  }
  return { status: "ok", label: "In range", delta: 0 };
}

export function targetStatusClass(status = "unknown") {
  if (status === "ok") return "border-emerald-200 bg-emerald-50 text-emerald-800 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-200";
  if (status === "low" || status === "high") return "border-amber-200 bg-amber-50 text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-200";
