...
import {
  compareToRange,
  formatTargetRange,
  formatTemperatureTargetRange,
  getStageEnvironmentTarget,
  targetStatusClass,
  temperatureFromFahrenheit,
...
function getEnvironmentLogTemperatureF(log = {}, preferredUnit = "F") {
  if (Number.isFinite(Number(log?.temperatureF))) {
    return Number(log.temperatureF);
  }
  if (Number.isFinite(Number(log?.temperatureC))) {
    return temperatureToFahrenheit(log.temperatureC, "C");
  }
  if (!Number.isFinite(Number(log?.temperature))) return null;
  return temperatureToFahrenheit(
    log.temperature,
    log.temperatureUnit || preferredUnit
  );
}

function getEnvironmentLogDisplayTemperature(log = {}, preferredUnit = "F") {
  const temperatureF = getEnvironmentLogTemperatureF(log, preferredUnit);
  if (temperatureF === null) return null;
  return temperatureFromFahrenheit(temperatureF, preferredUnit);
}

function statusLabelWithSuffix(comparison, suffix = "") {
  if (!comparison || comparison.status === "unknown") return "No target";
  if (comparison.status === "ok") return comparison.label;
  return `${comparison.label}${suffix}`;
}

function getSopChecklistFromGrow(grow = {}) {
  const source =
    grow?.sopChecklist ||
    grow?.sopChecklistItems ||
    grow?.workflowChecklist ||
    [];
  return Array.isArray(source) ? source : [];
}

function isSopWorkflowGrow(grow = {}) {
  return Boolean(
    grow?.workflowTemplateId ||
      grow?.sopTemplateId ||
      String(grow?.source || "").toLowerCase() === "sop-template" ||
      String(grow?.workflowSource || "").toLowerCase() === "sop-template" ||
      String(grow?.parentSource || "").toLowerCase() === "sop"
  );
}
...
export default function GrowDetail({
  ...
  useEffect(() => {
    const currentStage = String(grow?.stage || "").trim();
    if (!currentStage) return;
    setEnvInputs((current) =>
      current.stage ? current : { ...current, stage: currentStage }
    );
  }, [grow?.stage]);
  ...
  useEffect(() => {
    if (Array.isArray(logsFromProps)) return undefined;

    const user = auth.currentUser;
    if (!user || !growId) return undefined;
    ...
    const unsub = onSnapshot(
      ...
      (error) => {
        console.warn("Environment log subscription failed:", error);
      }
    );

    return () => unsub();
  }, [growId, logsFromProps]);
  ...
  const saveEnvLog = async () => {
    ...
    if (!Number.isFinite(temperatureNumber) || !Number.isFinite(humidityNumber)) {
      setPageNotice({
        tone: "error",
        message: "Enter valid temperature and humidity values.",
      });
      return;
    }

    const temperatureF = temperatureToFahrenheit(temperatureNumber, unit);
    const temperatureC = temperatureFromFahrenheit(temperatureF, "C");
    const newLog = {
      stage,
  ...
  const beginEditEnvLog = (log) => {
    const displayTemperature = getEnvironmentLogDisplayTemperature(log, unit);
    setEditLogId(log.id);
    setEditLog({
      stage: log.stage || "",
      temperature:
        displayTemperature === null ? "" : String(displayTemperature),
  ...
  const saveEditEnvLog = async () => {
    ...
    const temperatureNumber = Number(editLog.temperature);
    const humidityNumber = Number(editLog.humidity);
    if (!Number.isFinite(temperatureNumber) || !Number.isFinite(humidityNumber)) {
      setPageNotice({
        tone: "error",
        message: "Enter valid temperature and humidity values.",
  ...
  const sopChecklist = useMemo(
    () => getSopChecklistFromGrow(grow || {}),
    [grow]
  );
  const sopCompletedCount = useMemo(
    () => sopChecklist.filter((item) => item?.completed === true).length,
    [sopChecklist]
  );
  const sopProgressPercent =
    sopChecklist.length > 0
      ? Math.round((sopCompletedCount / sopChecklist.length) * 100)
      : 0;
  const sopWorkflowTitle =
    grow?.workflowTemplateTitle ||
    grow?.sopTemplateTitle ||
    grow?.workflowTitle ||
    "Workflow SOP";
  const showSopWorkflow = isSopWorkflowGrow(grow || {});

  const toggleSopChecklistItem = async (index) => {
    const next = sopChecklist.map((item, itemIndex) => {
    ...
    await callUpdateGrow({
      sopChecklist: next,
      sopChecklistUpdatedAt: new Date().toISOString(),
    });
  };

  const currentEnvironmentTarget = useMemo(
    () =>
      getStageEnvironmentTarget({
        prefs,
        stage: grow?.stage || "General",
        cultivationProfile: grow?.cultivationProfile || null,
      }),
    [prefs, grow?.stage, grow?.cultivationProfile]
  );
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      ...
      {showSopWorkflow ? (
        <section
          data-testid="grow-sop-origin"
          className="rounded-2xl border border-violet-200 dark:border-violet-900/60 bg-violet-50/70 dark:bg-violet-950/20 p-4 space-y-2"
        >
          <h2 className="text-lg font-semibold">SOP / Workflow Origin</h2>
          <div className="font-medium">{sopWorkflowTitle}</div>
          <div className="flex flex-wrap gap-2 text-xs">
            {grow?.workflowTemplateCategory ? (
              <span className="chip">{grow.workflowTemplateCategory}</span>
            ) : null}
            {grow?.workflowTemplateId || grow?.sopTemplateId ? (
              <span className="chip">
                ID: {grow.workflowTemplateId || grow.sopTemplateId}
              </span>
            ) : null}
            <span className="chip">Direct / SOP Start</span>
          </div>
          {grow?.sopTemplateSubtitle ? (
      ...
      {showSopWorkflow ? (
        <section
          data-testid="grow-sop-checklist"
          className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4 space-y-3"
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              <h2 className="text-lg font-semibold">SOP run checklist</h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                Track the operational quality gates for this specific grow.
              </p>
            </div>
            <span
              data-testid="grow-sop-checklist-progress"
              className="chip chip--active"
            >
              {sopProgressPercent}% complete
            </span>
          </div>
          ...
          {sopChecklist.length > 0 ? (
            <div className="space-y-2">
              {sopChecklist.map((item, index) => (
                <div
                  ...
                  className="rounded-xl border border-zinc-200 dark:border-zinc-700 p-3 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3"
                >
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {item?.stage ? <span className="chip">{item.stage}</span> : null}
                  ...
                  <button
                    type="button"
                    className={item?.completed ? "chip" : "chip chip--active"}
                    onClick={() => toggleSopChecklistItem(index)}
                  >
            ...
            <div className="text-sm text-zinc-500 dark:text-zinc-400">
              This SOP grow does not have checklist items saved.
            </div>
          )}
        </section>
      ) : null}
      ...
      <section className="space-y-4">
        <div
          data-testid="grow-environment-target"
          className="rounded-2xl border border-sky-200 dark:border-sky-900/60 bg-sky-50/70 dark:bg-sky-950/20 p-4"
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold">
                🌡️ {currentEnvironmentTarget.stage} Environment Target
              </h2>
              <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                Source: {currentEnvironmentTarget.source}
              </p>
            </div>
            <span className="chip">Current grow stage</span>
          </div>

          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 p-3">
              <div className="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">
                Temperature
              </div>
              <div className="mt-1 font-semibold">
                {formatTemperatureTargetRange(
                  currentEnvironmentTarget.target.tempMinF,
                  currentEnvironmentTarget.target.tempMaxF,
                  unit
                )}
              </div>
            </div>
            <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 p-3">
              ...
              <div className="mt-1 font-semibold">
                {formatTargetRange(
                  currentEnvironmentTarget.target.humidityMin,
                  currentEnvironmentTarget.target.humidityMax,
                  "%"
                )}
              </div>
            </div>
          ...
          {currentEnvironmentTarget.target.notes ? (
        ...
        <div>
          <h2 className="text-lg font-semibold">Environment Log</h2>
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Each reading is compared with the target saved for its selected stage.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
          <select
            data-testid="grow-environment-stage"
          ...
          <input
            data-testid="grow-environment-temperature"
            type="number"
            inputMode="decimal"
            step="0.1"
          ...
          />
          <input
            data-testid="grow-environment-humidity"
            type="number"
            inputMode="decimal"
            min="0"
            max="100"
          ...
          />
          <input
            data-testid="grow-environment-notes"
            type="text"
            placeholder="Notes"
        ...
        <button
          data-testid="grow-environment-save"
          onClick={saveEnvLog}
          className="px-4 py-1 rounded accent-bg disabled:opacity-60"
        ...
        {Array.isArray(logs) && logs.length > 0 ? (
          <div className="space-y-2 text-sm">
            {logs.map((log) => {
              const logTarget = getStageEnvironmentTarget({
                prefs,
                stage: log?.stage || "General",
                cultivationProfile: grow?.cultivationProfile || null,
              });
              const temperatureF = getEnvironmentLogTemperatureF(log, unit);
              const displayTemperature = getEnvironmentLogDisplayTemperature(
                log,
                unit
              );
              const temperatureComparison = compareToRange(
                temperatureF,
                logTarget.target.tempMinF,
                logTarget.target.tempMaxF
              );
              const humidityComparison = compareToRange(
                log?.humidity,
              ...
              return (
                <div
                  key={log.id}
                  data-testid="grow-environment-log"
                  ...
                  {editLogId === log.id ? (
                    ...
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1">
                          Temp: {displayTemperature ?? "—"}°{unit} | RH: {log.humidity}%
                        </div>
                        <div className="mt-2 flex flex-wrap gap-2">
                          <span
                            data-testid="environment-temperature-status"
                            className={`rounded-full border px-2.5 py-1 text-xs font-medium ${targetStatusClass(
                              temperatureComparison.status
                            )}`}
                          >
                            Temperature: {statusLabelWithSuffix(
                              temperatureComparison,
                              `°${unit}`
                            )}
                          </span>
                          <span
                            data-testid="environment-humidity-status"
                            className={`rounded-full border px-2.5 py-1 text-xs font-medium ${targetStatusClass(
                              humidityComparison.status
                            )}`}
                          >
                            Humidity: {statusLabelWithSuffix(
                              humidityComparison,
                              "%"
                            )}
                          </span>
                        ...
                        {log.notes ? (
                          <div className="mt-2 italic text-xs">{log.notes}</div>
                        ) : null}
                      </div>
          ...
          <div className="text-sm opacity-70">No environment logs yet.</div>
        )}
      </section>
    </div>
  );
