...
const tasksKey = (uid) => ["tasks", uid || "anon"];
const col = (uid) => collection(db, `users/${uid}/tasks`);

export function useTasks(uidProp) {
  const uid = uidProp || auth.currentUser?.uid || null;
  const queryClient = useQueryClient();
  const key = useMemo(() => tasksKey(uid), [uid]);

  const qFn = async () => {
    if (!uid) return [];
    const q = fsQuery(col(uid), orderBy("dueDate", "asc"));
  ...
  const addTask = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(key, [data, ...current.filter((t) => !t._optimistic)]);
    },
  });
  ...
  const updateTask = useMutation({
    ...
    onMutate: async ({ id, patch }) => {
      await queryClient.cancelQueries({ queryKey: key });
      const prev = queryClient.getQueryData(key) || [];
      queryClient.setQueryData(key, prev.map((t) => (t.id === id ? { ...t, ...patch } : t)));
      return { prev };
    },
