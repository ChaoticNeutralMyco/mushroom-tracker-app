...
export default function Dashboard({ grows: growsProp }) {
  ...
  // fallback Firestore load (if parent didn’t pass grows)
  useEffect(() => {
  ...
  // open from prefill (library -> new grow)
  useEffect(() => {
    try {
  ...
  const filteredGrows = useMemo(() => {
    ...
    if (stageFilter !== "All Stages") data = data.filter((g) => g.stage === stageFilter);
    if (dateRange.start) {
      data = data.filter((g) => g.createdAt && new Date(g.createdAt) >= new Date(dateRange.start));
    }
    if (dateRange.end) {
      data = data.filter(
        (g) =>
          g.createdAt &&
          new Date(g.createdAt) <= new Date(new Date(dateRange.end).getTime() + 86400000 - 1)
      );
    }
    return data;
  ...
  // ---------- Quick actions + batch handlers (optimistic + Firestore) ----------
  const uid = auth.currentUser?.uid || null;

  const onUpdateStatus = async (id, status) => {
    if (!id) return;
    // optimistic
    setGrowsLocal((prev) => prev.map((g) => (g.id === id ? { ...g, status } : g)));
    if (!uid) return;
    await updateDoc(doc(db, "users", uid, "grows", id), { status });
  };

  const onUpdateStage = async (id, nextStage) => {
    if (!id || !nextStage) return;
    // optimistic (merge nested stageDates)
    setGrowsLocal((prev) =>
      prev.map((g) =>
        g.id === id
          ? {
              ...g,
              stage: nextStage,
              stageDates: { ...(g.stageDates || {}), [nextStage]: new Date().toISOString() },
            }
          : g
      )
    );
  ...
  const onDeleteGrow = async (id) => {
    if (!id) return;
    setGrowsLocal((prev) => prev.filter((g) => g.id !== id));
    if (!uid) return;
